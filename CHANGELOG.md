...
- **Gameplay tuning:** tuned movement speed of **non‑boss** enemies — `weak`, `normal`, `strong`, `angel`, `giant` now have an effective movement speed of **~50** (spawn `speed` set to **62.5**; global ×0.8 applies).  This makes regular enemies noticeably slower than before but far more mobile than the earlier 30‑speed setting.
