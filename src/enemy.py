...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, x, y, enemy_type="basic", health=20, speed=100):
        super().__init__()
        self.x = x
        self.y = y
    ...
    def draw_enemy(self):
        ...
        if self.enemy_type.startswith("boss_"):
            ...
            boss_type = self.enemy_type.replace("boss_", "")
            asset_name = f"boss_{boss_type}.png"

        try:
            assets_dir = os.path.join(os.path.dirname(__file__), "..", "assets")
            image_path = os.path.join(assets_dir, asset_name)
        ...
        try:
            ...
            if (
                ...
                if game.prologo_final_boss_immortal and self.enemy_type == "boss_final":
                    ...
                    self.size_interp = min(self.size_interp, 1.0)
                    self.width = int(
                        self.original_width
                    ...
                    pulse = int((math.sin(self.shine_phase) + 1) / 2 * 150) + 20
                    ...
                    try:
                        ...
                        glow_radius = max(self.width, self.height)
                        pygame.draw.circle(
            ...
            if game and self.enemy_type in ["boss_big", "boss_final"]:
                ...
                if self.enemy_type == "boss_big":
                    if self.pattern_timer <= 0:
                        ...
                        self.pattern_timer = random.randint(140, 220)
                        ...
                        if distance > 0:
                            ...
                            for angle_offset in [-10, 0, 10]:
                                rad = base_angle + math.radians(angle_offset)
                                vel_x = math.cos(rad) * speed
                                vel_y = math.sin(rad) * speed
                                projectile = Projectile(
                                    self.x,
                                ...
                                game.enemy_projectiles.add(projectile)
                        self.big_shot_cooldown = random.randint(220, 320)
