...
class Game:
    ...
    def _init_entities(self) -> None:
        ...
        # victory overlay state (used for horde as well as generic wins)
        ...
        # Limbo Final boss defeat countdown (frames).  Starts when boss dies.
        self.limbo_final_victory_timer: int = 0
        # flag to ensure countdown only begins once per boss death
    ...
    def reset_run(self) -> None:
        ...
        *Game* object and counts as run-specific; carrying it between runs would
        ...
        # clear horde boss flags from previous run
        self.limbo_horde_boss_spawned = False
        self.limbo_horde_boss_killed = False
        ...
        self.limbo_horde_ready_for_victory = False
        # reset limbo final boss defeat timer
    ...
    def record_enemy_kill(self) -> None:
        ...
        # Historically we only counted kills while ``limbo_horde_active``
        # was True.  A subtle timing issue could leave ``limbo_horde_active``
        # cleared before the final boss died, which meant the boss kill would
        # be ignored and the level would never finish.  To avoid this we now
        # increment the kill counter whenever we're tracking a horde and the
        # event hasn't been marked completed yet.  The active flag is still
        # used by spawn logic but no longer gating the counter itself.
        try:
            ...
            if (
                ...
                # **however**, don't conclude the event until the scripted boss has
                # actually had a chance to spawn.  this prevents a quick player from
                # clearing every regular mob before the boss appears and then
                # immediately skipping the final wave.
                boss_spawned = getattr(self, "limbo_horde_boss_spawned", False)
                if self.limbo_horde_killed >= self.limbo_horde_initial and boss_spawned:
                    ...
                    try:
                        ...
                        pass
                    # Force-kill all remaining enemies and bosses so the victory
                    # condition (enemies_empty && bosses_empty) triggers immediately.
                    # Without this, stray giants or wave-bosses that spawned before
                    # the horde ended would keep the level alive indefinitely.
                    try:
                        # set health to zero first so any death effects or XP
                        # awards from update/collision logic still execute when
                        # ``kill()`` is called.  ``kill()`` typically removes the
                        # sprite from its groups, which ensures the ``enemies_empty``
                        # check in update() will succeed immediately.
    ...
    def update(self) -> None:
        ...
        # Fallback: if the event has been marked complete but for whatever
        # reason the "ready" flag was cleared or never set, still ensure we
        # begin the countdown once the room is empty.  This guards against
        # surprise exceptions or race conditions that might otherwise leave
        # the game running indefinitely.
        elif (
            getattr(self, "limbo_horde_completed", False)
            and not getattr(self, "showing_victory", False)
            ...
            if enemies_empty and bosses_empty:
                ...
                if getattr(self, "debug", False):
                    print("[LIMBO_HORDE] Victory timer fallback started")
        ...
        # Limbo Final boss defeated?  start countdown to stage-end screen
        if getattr(self, "limbo_final_victory_timer", 0) > 0:
            self.limbo_final_victory_timer -= 1
            if self.limbo_final_victory_timer <= 0:
                if getattr(self, "debug", False):
                    print("[LIMBO_FINAL] Victory timer expired, showing defeat screen")
                try:
